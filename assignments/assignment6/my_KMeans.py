    ...
    def initiate(self, X):
        ...
        if self.init == "random":
            ...
            for i in range(1, self.n_clusters):
                distance_squared = np.array([np.min([np.inner(points - value, points - value) for points in cluster_centers]) for value in X])
                individualProb = distance_squared / distance_squared.sum()
                allProbability = individualProb.cumsum()
                random = np.random.rand()

                for index, pValue in enumerate(allProbability):
                    if random < pValue:
                        i = index
                        break
