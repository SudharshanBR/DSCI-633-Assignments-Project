...
def stratified_sampling(y, ratio, replace=True):
    ...
    y_classes_ = list(set(y_array))
    labelIndices = {}
    for values in y_classes_:
            aIndex = np.where(y == values)
            labelIndices[values] = aIndex
    sSample = []
    for uniqueLabels in y_classes_:
        labelCount = np.ceil(len(ratio * labelIndices[uniqueLabels][0]))
        sSample.append(np.random.choice(labelIndices[uniqueLabels][0], int(labelCount), replace=replace))
    sSample = np.concatenate(sSample)
    return sSample.astype(int)