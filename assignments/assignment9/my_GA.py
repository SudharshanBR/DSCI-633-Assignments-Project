    ...
    def evaluate(self, decision):
        ...
        if decision not in self.evaluated:
            ...
            for fold in range(self.crossval_fold):
                ...
                test_indices = indices[int(np.ceil(self.data_X.shape[0] * 0.2)):]
                train_indices = indices[:int(np.ceil(self.data_X.shape[0] * 0.2))]
                ...
                if type(objs_crossval) == type(None):
                    objs_crossval = (objs * len(self.data_y))
                else:
                    objs_crossval += sum(objs) / self.crossval_fold
